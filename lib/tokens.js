...
class Tokens {
  ...
  //     qty: 1
  // }
  async sendTokens (output, walletInfo, bchUtxos, tokenUtxos, satsPerByte = 1.0) {
    try {
  ...
  async createTransaction (output, walletInfo, bchUtxos, tokenUtxos, satsPerByte = 1.0) {
    try {
      ...
      const {
        necessaryUtxos,
        change
      } = _this.sendBch.getNecessaryUtxosAndChange(bchOutput, bchUtxos, satsPerByte)
      ...
      //   output.qty
      // )
      const slpSendObj = await _this.getSendOpReturn(tokenUtxos, output.qty)
      // debugger
  ...
  async getSendOpReturn (tokenUtxos, sendQty) {
    try {
      const url = _this.bchjs.restURL
      const data = {
        tokenUtxos,
        sendQty
      }
      const options = {
        method: 'POST',
        url: `${url}slp/generatesendopreturn`,
        // url: 'http://localhost:3000/v3/slp/generatesendopreturn',
