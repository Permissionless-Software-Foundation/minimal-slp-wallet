...
class Tokens {
  ...
  async listTokensFromAddress (addr) {
    try {
      ...
      await _this.utxos.initUtxoStore2(addr)

      // const hydratedUtxos = await _this.utxos.hydrate(utxos)
      // console.log(`hydratedUtxos: ${JSON.stringify(hydratedUtxos, null, 2)}`)

      // console.log(`_this.utxos.utxoStore2: ${JSON.stringify(_this.utxos.utxoStore2, null, 2)}`)

      // Combine all Type 1, Group, and NFT token UTXOs. Ignore minting batons.
      const hydratedUtxos = _this.utxos.utxoStore2.slpUtxos.type1.tokens.concat(
