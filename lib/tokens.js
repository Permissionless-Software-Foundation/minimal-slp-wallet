...
class Tokens {
  ...
  // Burn all the SLP tokens associated to the tokenID
  async burnAll (
    tokenId,
    walletInfo,
    bchUtxos,
    ...
    try {
      ...
      // Get the necessary bch UTXOs to pay the fee
      const { necessaryUtxos } = _this.sendBch.getNecessaryUtxosAndChange(output, bchUtxos)

      // UTXOs array that contains the SLP UTXOs to burn
      // and the necessary BCH UTXOs to pay the fee
      const utxos = tokenUtxos.concat(necessaryUtxos)

      // Generate the transaction.
      const transaction = await _this.sendBch.createTransaction(
        output,
        walletInfo,
        utxos
      )
